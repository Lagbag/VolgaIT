...
from .grpc_consume_produce import \
    grpc_check_roles  # Подключение grpc_check_roles для использования в grpc_check_user_and_role
...
class IsDoctorOrLinkedPatientToVisit(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        if instance:
            return instance.patient_id == request.user.id or (
                        hasattr(request.user, 'role') and request.user.role.name == "Doctor")
        return False
